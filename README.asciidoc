...
** `\*diagnostics*` buffer has filetype `lsp-goto` so you can press `<ret>` on a line or use the `jump` command
...
* `lsp-sticky-contexts-enable` command to render sticky contexts at the top of the viewport window.
** `lsp_sticky_contexts_skip` option to specify the scope depth level to skip when rendering sticky contexts, default is `0`.
** `lsp_sticky_contexts_max` option to specify the max scope depth level to when rendering sticky contexts, default is `4`.
